...
// #pragma omp parallel for schedule(dynamic, 1) num_threads(num_compers)
    ...
    for (ui i = vertex_num + 1; i < input.size(); ++i)
    {
        ...
        // ######### WARNING: All edges have same label by default ##########
        eLabel label = 1; // For demostration purpose;
        // ##################################################################
...
{
    string key = "";
    key += "t " + std::to_string(size()) + ' ' + std::to_string(get_nedges()) + ' ';
    for (ui i = 0; i < size(); ++i)
    {
        // key += "v " + std::to_string(get_p_vertex(i)->id) + ' ' + std::to_string(get_p_vertex(i)->label) + '\n';
        ui degree = this->get_vertex_degree(i);
        key += "v " + std::to_string(get_p_vertex(i)->id) + ' ' + std::to_string(get_p_vertex(i)->label) + ' ' + std::to_string(degree) + ' ';
    }
    ...
    for(ui i = 0; i < get_nedges(); ++i)
    {
        ...
        key += "e " + std::to_string(edge2vertex[i].from) + ' ' + std::to_string(edge2vertex[i].to) + ' ';
    }
    return key;
}
